...
export function drawText(
	...
	lines.forEach((line, lineIndex) => {
		...
		line.characters.forEach(
			({ positionInAtlas, positionInLine, sizeInAtlas }) => {
				context.drawImage(
					fontImage,
					positionInAtlas[0],
					positionInAtlas[1],
...
function getFontWithColor(
	...
	if (!fontColorCache[fontAtlasPath].hasOwnProperty(color)) {
		...
		if (!context) {
			throw new Error(
				"Could not create context for font canvas. Perhaps this browser does not support the Canvas API."
			);
		}
...
function makeRenderLines(
	...
	return lineParts.map((linePart) => {
		...
		return Array.from(linePart).reduce(
			(line, character) => {
				...
				const positionInLine: Position = [
					...
					offsetInAtlas[1] +
						getVerticalOffsetForBaseline(metrics, baseline),
				];
...
function getHorizontalLineOffset(
	lineWidth: number,
	biggestWidth: number,
	align: TextAlign,
	direction: TextDirection
...
function getVerticalOffsetForBaseline(
	metrics: FontMetrics,
	baseline: TextBaseline
) {
	if (baseline === "top") {
